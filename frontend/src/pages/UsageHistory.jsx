...
const UsageHistory = () => {
  ...
  const avgDaily = trends.length > 0 ? Math.round(totalUsed / trends.length) : 0;
  const peakDay = trends.reduce((max, t) => t.total_used > max.total_used ? t : max, { total_used: 0 });
    ...
    <div className="animate-fade-in" data-testid="usage-history-page">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track your box consumption over time</p>
        </div>
        <div className="flex items-center gap-3">
          <Select value={timeRange} onValueChange={setTimeRange}>
            <SelectTrigger className="w-36 rounded-none" data-testid="time-range-select">
          ...
          <Select value={chartType} onValueChange={setChartType}>
            <SelectTrigger className="w-32 rounded-none" data-testid="chart-type-select">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card className="rounded-none border stat-card" data-testid="total-used-stat">
              <CardContent className="p-6">
                <p className="stat-label">Total Used</p>
                <p className="stat-value mt-2">{totalUsed}</p>
                <p className="text-xs text-muted-foreground mt-1">boxes in {timeRange} days</p>
              </CardContent>
            </Card>
            <Card className="rounded-none border stat-card" data-testid="avg-daily-stat">
              <CardContent className="p-6">
                <p className="stat-label">Daily Average</p>
                <p className="stat-value mt-2">{avgDaily}</p>
                <p className="text-xs text-muted-foreground mt-1">boxes per day</p>
              </CardContent>
            </Card>
            <Card className="rounded-none border stat-card" data-testid="peak-day-stat">
              <CardContent className="p-6">
          ...
          <Card className="rounded-none border" data-testid="usage-records-table">
            <CardHeader className="border-b">
              <CardTitle className="flex items-center gap-2">
                    ...
                    <TableBody>
                      {records.slice(0, 20).map((record) => (
                        <TableRow key={record.id} data-testid={`record-row-${record.id}`}>
                          ...
                          <TableCell className="font-medium">{record.box_name}</TableCell>
